...
#page(numbering: none)[
  ...
  #grid(
    columns: (3.5cm, 1fr),
    row-gutter: 3pt,
    [Студент:], [Одарчук О.Р., група МТ-22],
    [Тема роботи:], [«Розробка та програмна реалізація алгоритмів розв'язування математичних задач мовою Python»],
    [Варіант:], [21],
  )

  #v(1em)
  #text(weight: "bold")[Умови завдань:]
  #v(0.5em)

  #text(weight: "bold")[Завдання 1 (лінійний алгоритм):])
  #par(first-line-indent: 1.25cm)[
    Скласти блок-схему та програму лінійного алгоритму на мові Python.
    Знайти значення функції $ f = (y - z) dot frac(y - z^(y - z), 1 + x^2) + ln x $.
    Значення змінних x, y, z ввести з клавіатури.
  ]
  #v(0.5em)

  #text(weight: "bold")[Завдання 2 (розгалужений алгоритм):])
  #par(first-line-indent: 1.25cm)[
    Скласти програму для обчислення значення функції y(x):
    $
      y = cases(
    ...
    $
    Параметр $a = 3.1$. Значення x ввести з клавіатури.
  ]
  #v(0.5em)

  #text(weight: "bold")[Завдання 3 (циклічний алгоритм):])
  #par(first-line-indent: 1.25cm)[
    Обчислити функцію $ f(x) = sum_(k=1)^6 ( sin(0.17 x^k) / (2k + x) ) $ трьома способами:
    оператор циклу з параметром (for), з передумовою (while), з післяумовою (while True + break).
  ]
  #v(0.5em)

  #text(weight: "bold")[Завдання 4 (обробка списків):])
  #par(first-line-indent: 1.25cm)[
    Заповнити список з n елементів випадними цілими числами від 0 до 50.
    Визначити суму непарних елементів списку.
  ]
  #v(0.5em)

  #text(weight: "bold")[Завдання 5 (матриці та вектори):])
  #par(first-line-indent: 1.25cm)[
    Обчислити елементи матриці 5×5 за формулою
    $ a_(i,j) = (3 + i) / (i + j) dot sqrt(i^3 + j^2) + 2^(i - j) $.
    Використовуючи матрицю, обчислити вектор сум непарних рядків.
    Використати чотири підпрограми.
  ]
  #v(1.5em)

  #grid(
    columns: (1fr, 1fr),
    [Керівник: #underline[#h(120pt)]], [Дата: #underline[#h(80pt)]],
  )
]
...
#text(weight: "bold")[Змінні та типи даних.] Python є мовою з динамічною типізацією, що означає, що тип змінної визначається автоматично під час присвоювання значення. Основні типи даних: int (цілі числа), float (числа з плаваючою комою), str (рядки), bool (логічні значення).
...
Для роботи з двовимірними масивами (матрицями) у Python використовуються списки списків або спеціалізовані класи, як-то Matrix1Based, реалізований у даній роботі для зручної індексації від 1.
...
Python підтримує анотації типів (type hints), які дозволяють вказувати очікувані типи параметрів та поверненого значення. Це покращує читабельність коду та дозволяє статичним аналізаторам виявляти помилки на етапі написання коду.

Для роботи з математичними задачами часто використовуються функції з модуля math (sqrt, sin, cos, log, log10, asin тощо) та модуля random (randint, uniform, choice тощо).

== Особливості розв'язування математичних задач алгоритмічними методами

Розв'язування математичних задач алгоритмічними методами має ряд особливостей:
...
#text(weight: "bold")[Перевірка вхідних даних.] Перед виконанням обчислень необхідно перевіряти коректність вхідних даних: діапазон значень, типи даних, наявність ділення на нуль, від'ємні значення під коренем тощо.

#text(weight: "bold")[Візуалізація алгоритмів.] Блок-схеми є стандартизованим способом візуального представлення алгоритмів згідно з ДСТУ 19.701-90. Вони допомагають зрозуміти логіку алгоритму, виявити помилки та документувати програму.

#text(weight: "bold")[Модульність.] Розбиття програми на функції та модулі дозволяє повторно використовувати код, спрощує тестування та підтримку. У даній роботі реалізовано допоміжний модуль help.py з універсальними функціями введення даних та класом Matrix1Based.
...
#flowchart(
  caption: [Блок-схема основної функції third_task],
  diagram(
    node-stroke: dstu-stroke,
    spacing: (22mm, 10mm),
    mark-scale: 80%,
...
#flowchart(
  caption: [Блок-схема функції for_cycle_third_task],
  diagram(
    node-stroke: dstu-stroke,
    spacing: (20mm, 10mm),
    mark-scale: 80%,
...
#flowchart(
  caption: [Блок-схема функції while_cycle_third_task],
  diagram(
    node-stroke: dstu-stroke,
    spacing: (20mm, 10mm),
    mark-scale: 80%,
...
#flowchart(
  caption: [Блок-схема функції while_true_cycle_third_task],
  diagram(
    node-stroke: dstu-stroke,
    spacing: (22mm, 12mm),
    mark-scale: 80%,
...
#flowchart(
  caption: [Блок-схема функції fifth_task],
  diagram(
    node-stroke: dstu-stroke,
    spacing: (22mm, 10mm),
    mark-scale: 80%,
...
#flowchart(
  caption: [Блок-схема функції create_matrix_fifth_task],
  diagram(
    node-stroke: dstu-stroke,
    spacing: (20mm, 10mm),
    mark-scale: 80%,
...
#flowchart(
  caption: [Блок-схема функції create_vector_fifth_task],
  diagram(
    node-stroke: dstu-stroke,
    spacing: (8mm, 10mm),
    mark-scale: 80%,
...
#flowchart(
  caption: [Блок-схема функції print_matrix_fifth_task],
  diagram(
    node-stroke: dstu-stroke,
    spacing: (20mm, 10mm),
    mark-scale: 80%,
...
#flowchart(
  caption: [Блок-схема функції print_vector_fifth_task],
  diagram(
    node-stroke: dstu-stroke,
    spacing: (20mm, 10mm),
    mark-scale: 80%,
...
// #flowchart(
//   caption: [Блок-схема функції input_variables],
//   diagram(
//     node-stroke: dstu-stroke,
//     spacing: (5mm, 12mm),
//     mark-scale: 80%,
...
// #flowchart(
//     caption: [Блок-схема функції process_line],
//     diagram(
//     node-stroke: dstu-stroke,
//     spacing: (20mm, 12mm),
//     mark-scale: 80%,
...
Під час виконання першого завдання було реалізовано лінійний алгоритм обчислення складної математичної функції з перевіркою коректності вхідних даних. Блок-схема алгоритму наочно демонструє послідовність обчислень проміжних виразів та перевірки умови.

Друге завдання присвячено розгалуженому алгоритму — програмі обчислення кусково-заданої функції з трьома гілками обчислень залежно від значення аргументу. Реалізація з використанням конструкції if-elif-else забезпечує коректний вибір відповідної формули.

Третє завдання продемонструвало три способи реалізації циклічного алгоритму: через цикл for (з лічильником), цикл while (з передумовою) та цикл while True з оператором break (з післяумовою). Усі три підходи дали ідентичні результати, що підтверджує коректність реалізації.

Четверте завдання показало ефективність роботи зі списками у Python: генерація випадкових чисел, фільтрація елементів за умовою та обчислення суми за допомогою спискових включень (list comprehensions).

П'яте завдання включало роботу з двовимірними масивами (матрицями), обчислення елементів за формулою, формування вектора сум непарних рядків та структуроване використання чотирьох окремих підпрограм. Реалізований клас Matrix1Based забезпечив зручну індексацію від 1, що відповідає математичній нотації.
